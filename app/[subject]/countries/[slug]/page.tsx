...
import { hasCountriesHub, kickerForCountry, type CountriesData, type CountryItem } from '@/utils/countries'
...
function findItem(data: CountriesData, slug: string): CountryItem | null {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) return {}
  return {
    title: `${item.title} — AP Comp Gov Countries | geta5.app`,
...
export async function generateStaticParams() {
  try {
    const file = path.join(process.cwd(), 'public', 'data', 'ap-comparative-government', 'countries.json')
    const raw = fs.readFileSync(file, 'utf-8')
    const data = JSON.parse(raw) as CountriesData
    return data.items.map(i => ({ subject: 'ap-comparative-government', slug: i.id }))
  } catch {
    return []
  }
}
...
export default async function CountryDetailPage({ params }: PageProps) {
  ...
  if (!subject || !hasCountriesHub(subjectSlug)) notFound()
  const data = loadData(subjectSlug)
  const item = data ? findItem(data, slug) : null
  if (!item) notFound()

  const kicker = kickerForCountry(item)
    ...
    <AuthGuard requireAuth>
      <div style={{
        ...
        <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
          ...
          <Link href={`/${subjectSlug}/countries`} style={{ color: 'var(--text-secondary)', textDecoration: 'none' }}>Countries</Link>
          <span style={{ margin: '0 8px' }}>/</span>
          {item.title}
        </div>
        ...
        <div style={{
          ...
          <MetaCell label="Key Institutions" value={item.key_institutions.join(' · ')} />
          <MetaCell label="Comparative Themes" value={item.comparative_themes.join(' · ')} />
          <MetaCell
                ...
                <span>
                  {item.paired_with.map((p, idx) => (
                    <span key={p.id}>
                      {idx > 0 && ' · '}
                      <Link
                        href={`/${subjectSlug}/countries/${p.id}`}
          ...
          {item.required_cases.length > 0 && (
            <MetaCell label="Required cases / examples" value={item.required_cases.join(' · ')} />
          )}
        </div>

        {/* Country sub-sections */}
        {item.sections.length > 0 && (
          <CountrySections
            heading="Country breakdown — five dimensions"
