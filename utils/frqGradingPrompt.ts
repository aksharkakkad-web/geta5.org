...
CONNECTION PARITY: If the response demonstrably contains the comparison, connection, or causal explanation the rubric requires — even if the wording differs from the rubric's example phrasing — credit the point. Before denying with "lacks a clear comparison" / "lacks connection" / "doesn't connect well" / "did not provide a comparison," locate the specific sentence(s) where the student made the connection. If you can quote those sentences, the connection IS present and the point is earned. If you genuinely cannot, the connection is missing. Examples of equivalent forms that count as connections: "coalitions of citizens" = "organized groups"; "66% of Democrats vs 45% of Republicans" = a comparison of both parties; "X led to Y" / "this caused" / "because of X" = a causal explanation; "differs from" / "in contrast" / "unlike" = a comparison statement. Synonym differences and paraphrasing are not deficits — they are normal student writing. Self-check before earned=0: can you point to the literal sentences that fail the rubric? If your only complaint is that the student used different words than the rubric example, credit the point.

DENIAL VOCABULARY BLOCKLIST: When the student wrote a substantive response (≥50 words and on-topic), the following denial reasons are FORBIDDEN unless you cite the SPECIFIC factual error or off-topic content: "lacks clarity," "lacks accuracy" (without naming the inaccuracy), "lacks specificity" (when specific names/numbers/events ARE present), "vague" (when concrete examples ARE present), "doesn't connect well," "didn't connect deeply enough," "needs more depth." If you find yourself reaching for one of these phrases on a substantive response, that is a signal to RECONSIDER and credit the point. The valid reasons to deny on a substantive response are: factually wrong claim (cite the error), wrong region/period/scope (cite what's out of scope), missing literal rubric requirement (cite which specific element from the rubric criterion the response failed to address), or addressed a different prompt (cite what the student answered instead). If your reasoning would have to be one of the BLOCKLIST phrases above, the point should earn.

ESSAY ROW ANCHORS (LEQ, DBQ, argument_essay, essay): Before denying any of these specific essay rows, check the response for the literal markers:
- THESIS row: earns when the response contains a defensible position + at least one reason. "X is the most influential because Y" / "X transformed Z due to A and B" / "the most significant change was X because Y" all earn. Do NOT require sophistication.
- CONTEXTUALIZATION row: earns when the response describes broader historical context (named era, ongoing process, or pre-existing conditions) connected to the prompt. A paragraph naming Industrial Revolution / Enlightenment / Cold War / Berlin Conference / Columbian Exchange + brief explanation of relevance earns.
- EVIDENCE row(s): earns when the response names specific historical persons/events/policies/dates and uses them to support the thesis. Two such named items connected to the thesis = 2 evidence points on a 2-point evidence row. Do NOT require the student to use the rubric's specific examples — any defensible specific evidence earns.
- REASONING row: earns when the response explicitly traces causation, comparison, or change-over-time linking evidence to thesis. "X led to Y led to Z" / "unlike A, B did C" / "this caused" / "the result was" all signal reasoning. Do NOT require formal historical-thinking-skill vocabulary.
- COMPLEXITY row: earns when the response addresses an opposing view, alternative interpretation, or a nuance that complicates the thesis — even briefly. "Although critics argue X, the stronger case is Y because Z" / a paragraph framed as "another factor" with rebuttal / explicit acknowledgment of trade-offs all earn.
