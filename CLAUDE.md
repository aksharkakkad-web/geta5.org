...
- 2026-03-30: UI overhaul — darker #050508 base with blue undertone, Outfit font (next/font/google), auto-hide glass header, hero animation (staggered reveal + particles), 7-effect visual system (glass cards, neon glow, ambient blobs, gradient borders, deep shadows, particles, micro-interactions), 3D icons via Three.js (@react-three/fiber), page transitions, scroll-triggered reveals via Intersection Observer, parallax on card art headers, confetti on good scores (canvas-confetti), counter roll-up on score rings, streak flame flicker
